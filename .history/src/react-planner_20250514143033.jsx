...
class ReactPlanner extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    let { stateExtractor, state, /* projectActions, */ catalog } = nextProps; // projectActions from nextProps is no longer used here
    let plannerState = stateExtractor(state);
    let catalogReady = plannerState.getIn(["catalog", "ready"]);

    // The following block was causing the infinite loop and has been removed/commented out.
    // if (!catalogReady) {
    //   projectActions.initCatalog(catalog);
    // }

    // Check if the catalog prop has actually changed before dispatching
    if (nextProps.catalog && this.props.catalog !== nextProps.catalog) {
      // If using Immutable.js, a more robust check might be:
      // if (nextProps.catalog && !this.props.catalog.equals(nextProps.catalog)) {
      this.props.projectActions.initCatalog(nextProps.catalog); // Use this.props.projectActions
    }
  }
